/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PaystackPaymentWhereInput } from "./PaystackPaymentWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { PaystackPaymentOrderByInput } from "./PaystackPaymentOrderByInput";

@ArgsType()
class PaystackPaymentFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => PaystackPaymentWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => PaystackPaymentWhereInput, { nullable: true })
  @Type(() => PaystackPaymentWhereInput)
  where?: PaystackPaymentWhereInput;

  @ApiProperty({
    required: false,
    type: [PaystackPaymentOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [PaystackPaymentOrderByInput], { nullable: true })
  @Type(() => PaystackPaymentOrderByInput)
  orderBy?: Array<PaystackPaymentOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { PaystackPaymentFindManyArgs as PaystackPaymentFindManyArgs };
