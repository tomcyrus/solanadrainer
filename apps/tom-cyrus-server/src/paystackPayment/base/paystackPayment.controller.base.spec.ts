...
const service = {
  ...
  paystackPayments: () => FIND_MANY_RESULT,
  paystackPayment: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("PaystackPayment", () => {
  ...
  test("POST /paystackPayments", async () => {
    await request(app.getHttpServer())
      .post("/paystackPayments")
      .send(CREATE_INPUT)
  ...
  test("GET /paystackPayments", async () => {
    await request(app.getHttpServer())
      .get("/paystackPayments")
  ...
  test("GET /paystackPayments/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/paystackPayments"}/${nonExistingId}`)
  ...
  test("GET /paystackPayments/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/paystackPayments"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /paystackPayments existing resource", async () => {
